...
class Test extends React.Component {
  ...
  response = (quesNo) => {
    ...
    if (result === false) {
      localStorage.setItem(
        'questionNo',
        JSON.stringify(questionNo.concat(quesNo))
      )
    }
  ...
  setTime = () => {
    const examTime =
      Date.parse(new Date()) + 1000 * 60 * 60 - Date.parse(new Date())
        ...
        {!!authenticated || !!guest ? (
          <div id='test'>
            {this.state.submit === true ? (
              <Redirect to='/submit-response' />
            ) : null}
            {this.state.timeOut === true ? <Redirect to='/timeout' /> : null}

            <Header
              examTime={this.remainingTime()}
              submit={this.handleSubmit}
            />
            <Paper square elevation={0} className={classes.header}>
              <Typography>
                {Questions[activeStep].id}. {Questions[activeStep].label}
              </Typography>
            </Paper>
